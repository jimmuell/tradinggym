...
serve(async (req) => {
  ...
  try {
    ...
    const matched = subs.data.find((s: Stripe.Subscription) =>
      s.items.data.some((it: Stripe.SubscriptionItem) => it.price.id === proPriceId || it.price.id === expertPriceId)
    );
    ...
    const planTier = matched.items.data.find((it: Stripe.SubscriptionItem) => it.price.id === proPriceId)
      ? "pro"
      : "expert";
