...
import { X, CheckCircle2, AlertTriangle, Pencil, Trash2, Plus, RotateCcw } from 'lucide-react';
import {
...
export function ChecklistDrawer({ open, onClose }: Props) {
  ...
  useEffect(() => {
    if (open && templates && templates.length === 0 && !seed.isPending) {
      seed.mutate();
    }
  }, [open, templates, seed]);

  // Bind to today's template
  useEffect(() => {
  ...
  const updatePrep = async (itemId: string, value: string | boolean | number, item: ChecklistItem) => {
  ...
  const resetToDefault = async () => {
    if (!activeTemplate) return;
    // Delete all default templates and reseed
  ...
  return (
    <>
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {activeTemplate?.strategy_name ?? 'Loading…'}
            </p>
          </div>
                  ...
                  {!emotionalReadiness && (
                    <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3 space-y-2">
                      <div className="flex gap-2">
                        ...
                        <p className="text-xs">
                          You've indicated you're not in the right headspace. Traders
                          who proceed in this state lose significantly more often.
                        </p>
                      </div>
                      <div className="flex gap-2">
        ...
        <footer className="border-t border-border p-3 flex items-center justify-between shrink-0">
          <Button
            variant="ghost"
            size="sm"
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This is a core step from the {activeTemplate?.strategy_name} blueprint.
              Removing it may reduce your trading accuracy. Are you sure?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
